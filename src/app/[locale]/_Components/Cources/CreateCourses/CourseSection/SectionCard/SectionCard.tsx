...
import { setAddedOrder, setAddedSection } from "@/store/redux/createcourse/createcourseslice"
...
export function SectionCard() {
    ...
    async function handleSubmitSection(value: number, idx: number) {
        ...
        const data: CreateSectionType = {
            title: payload,
            order: String(value),
            courseId: createCorseStore.courseId,
        }
        ...
        <section>
            ...
            <div className="space-y-4 md:w-3/4 mx-auto">
                {NUMOFSECTIONS.map((value, idx) =>
                    <Card key={value} className="  gap-0 px-3">
                        <div className="flex w-full gap-4">
                            ...
                            <div className="flex-1">
                                <CardHeader className="flex justify-between space-y-3 px-0">
                                    ...
                                    <FontAwesomeIcon onClick={() => handleRemoveSection(value, idx)} className=" select-none cursor-pointer text-(--text-muted) hover:text-red-500" icon={faTrashCan} />
                                </CardHeader>
                                <CardContent className="px-0">
                                    <Input key={value} ref={(element) => { sectionData.current[idx] = element }} className="INPUT_STYLE py-5" placeholder="Enter Section Title" />
                                </CardContent>
                            </div>
                        </div>
                        <CardFooter className="flex justify-end px-0">
                            <Button onClick={() => handleSubmitSection(value, idx)} className={BUTTON_STYLE + " cursor-pointer  py-1/4 rounded-sm "} >
                                Add
                            </Button>
                        </CardFooter>
                    </Card>
